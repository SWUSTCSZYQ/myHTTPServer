...
int ThreadPool::threadpool_create(int _thread_count, int _queue_size) {
    bool err = false;
    do {
        if(_thread_count <= 0 or _thread_count > MAX_THREADS or _queue_size <= 0 or _queue_size > MAX_QUEUE)
        {
            _thread_count = 4;
            _queue_size = 1024;
        }
...
int ThreadPool::threadpool_destroy(ShutDownOption shutdown_option) {
    ...
    do {
        ...
        if(pthread_cond_broadcast(&notify) != 0 or pthread_mutex_unlock(&lock) != 0)
        {
            err = THREADPOOL_LOCK_FAILURE;
            break;
        }
...
void *ThreadPool::threadpool_thread(void *args) {
    while(true)
    {
        ThreadPoolTask task;
        pthread_mutex_lock(&lock);
        while(count == 0 and !shutdown)
        {
            pthread_cond_wait(&notify, &lock);
        }
        if(shutdown == immediate_shutdown or (shutdown == graceful_shutdown and count == 0))
        {
            break;
        }
